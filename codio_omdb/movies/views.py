# from django.shortcuts import render		 # no templates
import urllib.parse
...
from codio_omdb.celery import app
from movies.models import Movie
from movies.tasks import search_and_save

def search(request):
    search_term = request.GET["search_term"]
    res = search_and_save.delay(search_term)		# delay => celery task. Without delay => call as normal function
    try:
        res.get(timeout=2)
    except TimeoutError:
        return redirect(
...
def search_wait(request, result_uuid):
    ...
    try:
        res.get(timeout=-1)							# 0: wait forever, -1: return immediately
    except TimeoutError:
